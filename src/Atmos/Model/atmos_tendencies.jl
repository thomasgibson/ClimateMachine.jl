...
eq_tends(::AbstractEquationsForm,
...
# Mass
eq_tends(::Mass, ::Anelastic1D, ::Union{Flux{FirstOrder}, FluxDifferencing{FirstOrder}}) = ()
eq_tends(::Mass, ::Compressible, ::Union{Flux{FirstOrder}, FluxDifferencing{FirstOrder}}) = (Advect(),)

# Momentum
eq_tends(pv::Momentum, ::Compressible, ::Union{Flux{FirstOrder}, FluxDifferencing{FirstOrder}}) =
    (PressureGradient(),)
eq_tends(pv::Momentum, ::Anelastic1D, ::Union{Flux{FirstOrder}, FluxDifferencing{FirstOrder}}) = ()

# Energy
eq_tends(::Energy, m::TotalEnergyModel, ::Union{Flux{FirstOrder}, FluxDifferencing{FirstOrder}}) = (Advect(), Pressure())
eq_tends(::ρθ_liq_ice, m::θModel, ::Union{Flux{FirstOrder}, FluxDifferencing{FirstOrder}}) = (Advect(),)
...
eq_tends(::Unsplit, pv::AbstractEnergyVariable, m::AtmosModel, tt::Flux{FirstOrder}) = (
    eq_tends(pv, energy_model(m), tt)...,
    eq_tends(pv, radiation_model(m), tt)...,
)

# AbstractMoistureVariable
eq_tends(::Unsplit, ::AbstractMoistureVariable, ::AtmosModel, ::Flux{FirstOrder}) =
  (Advect(),)
...
# Tracers
eq_tends(::Unsplit, pv::Tracers{N}, ::AtmosModel, ::Flux{FirstOrder}) where {N} =
  (Advect(),)

##### CentralSplitForm flux first order tendencies
# treat everything using flux differencing

eq_tends(::CentralSplitForm,
         pv::AbstractPrognosticVariable,
         atmos::AtmosModel,
         ::FluxDifferencing{FirstOrder}) = eq_tends(Unsplit(), pv, atmos, Flux{FirstOrder}())
...
eq_tends(::KennedyGruberSplitForm,
         pv::AbstractPrognosticVariable,
         atmos::AtmosModel,
         tt::Flux{FirstOrder}) = eq_tends(Unsplit(), pv, atmos, tt)

# Mass
eq_tends(::KennedyGruberSplitForm, pv::Mass, atmos::AtmosModel, tt::Flux{FirstOrder}) = ()
eq_tends(::KennedyGruberSplitForm, pv::Mass, atmos::AtmosModel, tt::FluxDifferencing{FirstOrder}) =
    (eq_tends(pv, compressibility_model(atmos), tt))

# Momentum
eq_tends(::KennedyGruberSplitForm, pv::Momentum, m::AtmosModel, tt::Flux{FirstOrder}) =
  eq_tends(pv, compressibility_model(m), tt)
eq_tends(::KennedyGruberSplitForm, pv::Momentum, m::AtmosModel, ::FluxDifferencing{FirstOrder}) =
  (Advect(),)
...
eq_tends(::KennedyGruberSplitForm,
         ...
         m::AtmosModel,
         tt::Flux{FirstOrder}) = eq_tends(pv, radiation_model(m), tt)
eq_tends(::KennedyGruberSplitForm,
         pv::AbstractEnergyVariable,
         m::AtmosModel,
         tt::FluxDifferencing{FirstOrder}) = eq_tends(pv, energy_model(m), tt)
